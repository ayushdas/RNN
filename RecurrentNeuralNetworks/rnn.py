...
class RNN(object):
	...
	def acc_deltas_bptt(self, x, d, y, s, steps):
		...
		deltaV_sum = np.zeros((self.hidden_dims, self.vocab_size))
		deltaW_sum = np.zeros((self.vocab_size, self.hidden_dims))
		lst = [t * (np.ones(len(t)) - t) for t in s]
		gradbptt_in = np.array(lst)
		for t in reversed(range(len(x))):
	...
	def generate_loss_on_sequence(self, start):
		# print(start)
		# sequence = [start]
		# x = [start]
		x = [start[0]]
...
if __name__ == "__main__":
	...
	if mode == "train-lm":
		...
		lr = 0.5
		print('#'*10)
		print('Learning Rate: ',lr,' Number of Look Back Steps: ',0,' Number of Hidden Units: ',hdim)
		train_RNN = RNN(vocab_size, hdim, vocab_size)
		run_loss = train_RNN.train(X=X_train, D=D_train, X_dev=X_dev, D_dev=D_dev, epochs=epoch_nums, learning_rate=lr)					
		...
		print('#'*10)
		np.save('rnn.U1.npy',train_RNN.U)
		np.save('rnn.V1.npy', train_RNN.V)
		np.save('rnn.W1.npy', train_RNN.W)
		########################## Loop for Q2a ##########################
		lrs = [0.5,0.1,0.05]
		hdims = [25,50]
		loopbacks = [0,2,5]
	...
	if mode == "generate_from_lm":
		...
		print('Hdims: ',hdim) # 25
		sequence = [word_to_num['car'],word_to_num["bus"],word_to_num["height"]]#,word_to_num['itself']]#,word_to_num['the'],word_to_num['tree']]
		seq = r.generate_loss_on_sequence(sequence)
